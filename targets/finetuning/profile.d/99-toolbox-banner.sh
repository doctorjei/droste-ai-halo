...
# (targets/finetuning/build-spec:100). Reading DROSTE_FINETUNING_PORT here would
# find nothing, silently, for ever.
...
# own finetuning.cfg, several hundred lines of their settings. So this asks
...
serve_port() {
  ...
  if [[ -f "$file" && -r "$file" ]]; then
    pv=$(
      ...
      droste::cfg_get DROSTE_JUPYTER_PORT "$file"
    ) || pv=""
  fi
...
# DROSTE_JUPYTER_HOST is a user setting now, so a box bound to one interface
...
# catastrophic in an --ip flag, which would then bind loopback ONLY.
...
# DROSTE_JUPYTER_HOST names one interface.
