...
#     snapshot path to use as DROSTE_DS4_MODEL in /opt/config/ds4.cfg.
...
Targets:
  ...
  mtp  Optional speculative decoding component, about 3.5 GB on disk.
       ...
       enabled explicitly (DROSTE_DS4_MTP in /opt/config/ds4.cfg, or --mtp).
...
if [ "$MODEL" = "mtp" ]; then
    ...
    echo "Enable it in /opt/config/ds4.cfg:"
    for p in $PATHS; do
    ...
    echo "Point ds4-server at the model in /opt/config/ds4.cfg:"
    for p in $PATHS; do
