...
# own ds4.cfg, several hundred lines of their settings. So this asks
...
# (8000: ds4-server's own) instead of printing garbage or failing the login
...
serve_port() {
  local def=8000 file="${DROSTE_SERVE_ENV:-/opt/data/ds4.cfg}" pv=""
  if [[ -f "$file" && -r "$file" ]]; then
    pv=$(
      ...
      droste::cfg_get DROSTE_DS4_PORT "$file"
    ) || pv=""
  fi
...
# DROSTE_DS4_HOST is a user setting now, so a box bound to one interface would
...
# DROSTE_DS4_HOST names one interface.
