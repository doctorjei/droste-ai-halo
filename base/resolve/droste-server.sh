...
# existing machinery make reality match. It does NOT touch STARTUP_ENABLED in
# server.env — Jei's ruling: a user who starts a server by hand asked to start a
# server, not to change what the box does at boot. That asymmetry is exactly what the
# two-setting split buys, and it is why `stop` is always temporary: the next container
# start resets .IS_ACTIVE from STARTUP_ENABLED and the server comes back.
#
...
Usage: server_start | server_stop | server_restart | server_status
  ...
  stop     ask for it to stop (TEMPORARY — it comes back at the next box start
           if STARTUP_ENABLED=1 in ${DROSTE_SERVE_ENV})
  restart  stop, then start
  status   what the box wants, and what is actually true

To change what happens at BOX START, edit STARTUP_ENABLED in
...
status_service() {
    ...
    printf '  at box start          : %s   (STARTUP_ENABLED in %s)\n' \
        "$([ "${SERVE_STARTUP_ENABLED:-0}" -eq 1 ] && printf yes || printf no)" "$DROSTE_SERVE_ENV"
    printf '  port                  : %s\n' "${SERVE_PORT:-<unset>}"
    if serve::state_ok; then
    ...
    if [ "$want" = no ] && [ -f "$DROSTE_SERVE_ACTIVE" ] && [ "${SERVE_STARTUP_ENABLED:-0}" -eq 1 ]; then
        ...
        printf 'with the box. To keep it down, set STARTUP_ENABLED=0 in %s.\n' "$DROSTE_SERVE_ENV"
