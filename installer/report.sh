...
write_notes() {
  ...
  local f=$EMIT_DIR/NOTES.md box port bs hs conf data pcache note
  # 🚨 FOUND BY RUNNING THE DRY RUN, NOT BY READING IT. The static half of
  # check-installer-dryrun.sh had dropped redirect detection to kill a wave of
  # false positives, and this file is written by a `} > "$f"` — so the first
  # real `--dry-run` died here, on a NOTES.md whose directory it had correctly
  # declined to create. ⭐⭐ THE LESSON IS THE CHECKER'S: a static rule tuned for
  # quiet stops asserting the thing it was tuned around. The redirect detector
  # came back, narrowed to unquoted redirects, and the runtime assertion exists
  # because tuning will happen again.
  dry::skip "write $f" && return 0
  {
    ...
    printf '  box%s config dir and launches the service on the port recorded there.\n' "'s"
    ...
    printf '\n## Your installation\n\n'
    printf '| Box | Start w Box | Start w Host | Port | Config dir | Program dir | Cache dir |\n'
    printf '|---|---|---|---|---|---|---|\n'
    for box in "${SELECTED[@]}"; do
      port=$(box_port_disp "$box")
      bs=$(yn_word "$(box_boxsv "$box")")
      hs=$(yn_word "$(box_hstsv "$box")")
      # A box carried over from an older layout has no config or cache dir of its
      # own to report (nothing bound one); it says so rather than naming a path
      # the ini does not carry. The config column is the one a reader needs most
      # on such a box — it is where their settings will live from now on, and it
      # is not where they are today.
      ...
      printf '| %s | %s | %s | %s | %s | %s | %s |\n' \
        "$box" "$bs" "$hs" "$port" "$conf" "$data" "$pcache"
    done
    ...
    # ⭐ THE QUESTION THAT SEPARATES THEM IS "WHAT GETS IT BACK?" (Jei, s79) —
    # nothing, at the top; a reinstall or a re-run, in the middle; the program
    # rebuilds it unasked, at the bottom. Said in that order, on purpose.
    ...
    printf '  losing work, but getting it back means installing or re-running —\n'
    ...
    printf '  A box set up before the program dir held the Python environment\n'
    printf '  overlay keeps it here; that one directory is excluded from the\n'
    printf '  wipe, and the run says where it is.\n'
    printf -- '- Your INPUT, OUTPUT, WORKSPACE and saved WORKFLOWS sit beside the\n'
    printf '  config and program dirs, under the same per-box directory, and are\n'
    printf '  bound straight into the app. droste-setup.sh deletes nothing there\n'
    printf '  unasked: the one way it ever does is the "remove data at new path"\n'
    ...
    printf 'EVERY box start. droste-setup.sh writes that file before the box has\n'
    printf 'ever run, only when it is absent, and it is YOURS from then on; it\n'
    printf 'also documents every other\n'
    ...
    for box in "${SELECTED[@]}"; do
      conf="${PATHS["$box:config"]:-${EXD_PATH["$box:config"]:-<config-dir>}}"
      printf '    %s/%s\n' "$conf" "${BOX_CFG[$box]}"
    ...
    printf 'says so in `<program dir>/logs/<box>-serve.log`) and reports UNHEALTHY —\n'
    ...
    printf '\n## Per-box quickstart\n'
    for box in "${SELECTED[@]}"; do
      # Every path named in this block is a CONFIG file, so it is the config dir
      # that is resolved here — the program dir holds nothing anyone is told to
      # open before a first run.
      data="${PATHS["$box:config"]:-${EXD_PATH["$box:config"]:-<config-dir>}}"
      port=$(box_port_disp "$box")
      printf '\n### %s — %s\n\n' "$box" "${BOX_PITCH[$box]}"
      ...
      case "$box" in
        ...
        finetuning)
          ...
          printf "    podman exec %s grep -o 'token=[0-9a-f]*' /opt/program/logs/finetuning-serve.log | tail -1\n" \
            "$(box_ctr finetuning)"
    ...
    printf '\nRecreating replaces the container; your config dir, your program\n'
    printf 'dir, your cache dir and the in-box overlays under them are untouched —\n'
    printf 'that is what makes in-box installs survive a recreate (pip packages\n'
    printf 'and custom nodes both ride overlays in the program dir).\n'
    ...
    for box in "${CONFIGURE[@]}"; do
      ...
      printf -- '- %s: port=%s, box-start=%s, host-boot=%s, config=`%s`, program=`%s`, cache=`%s`, %s.%s\n' \
        "$box" "${CFG_PORT[$box]}" "$(yn_word "${CFG_BOXSV[$box]:-}")" \
        "$(yn_word "${CFG_HSTSV[$box]:-}")" "${PATHS["$box:config"]}" \
        "${PATHS["$box:program"]}" "${PATHS["$box:pcache"]}" "$note" \
...
dash_table() {   # with-on(0|1)
  local on=$1 hl=$DASH_HOST_KEY box st stg g note data conf row port budget head
  local -a gcol=() glyph=()
