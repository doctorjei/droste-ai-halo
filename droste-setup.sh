...
# the port the service BINDS: droste-setup writes it into server.env and the
...
# droste-setup wires podman's healthcheck at CREATE time (the images carry no
...
QUIT_ARMED=0
quit_now() {
  printf '\n  %sExiting droste-setup — no further changes made.%s\n\n' \
    "$C_TEXT" "$RESET"
  exit 0
}
...
# what droste-setup last wrote. Parsed the same defensive way P1's serve library
...
emit_serve_env() {  # box
  ...
  mkdir -p "$(dirname "$f")" 2>/dev/null || :
  {
    ...
    printf '# Written by droste-setup on %s; safe to edit by hand:\n' "$(date +%F)"
...
write_notes() {
  local f=$EMIT_DIR/NOTES.md box port bs hs data note
  {
    ...
    printf 'file survives image updates and box recreation. droste-setup writes\n'
    ...
    printf 'droste-setup enables it for you when a box asks for host boot (no\n'
