...
def _restricted_unpickle_keys(fileobj):
    ...
    real class -- it returns an inert stand-in; persistent_load (torch
    tensor storages) also returns one. Every REDUCE / NEWOBJ therefore
    lands on the inert stub, not attacker code. The stub only records dict
    keys assigned to it (SETITEM/SETITEMS) so a state dict's tensor-name
    keys survive. Plain-dict pickles come back as a real dict and we read
    their keys directly.

    The stand-in is a dynamically-built TYPE, not an instance: NEWOBJ /
    NEWOBJ_EX (what protocol >= 2 emits for an ordinary object, i.e. what
    torch.save writes when a whole MODEL was serialized rather than a state
    dict) require a real type and abort the load otherwise. Subclassing per
    (module, name) satisfies the opcodes while importing nothing."""
    recorded = set()
