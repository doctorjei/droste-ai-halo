#!/usr/bin/env python3
"""droste-hf-adopt.py: adopt already-downloaded model files into the HF hub cache.
...
examples:
  ...
  droste-hf-adopt.py --repo Qwen/Qwen2.5-0.5B-Instruct-GGUF \\
      ~/Downloads/qwen2.5-0.5b-instruct-q4_k_m.gguf
  droste-hf-adopt.py --apply --repo Qwen/Qwen2.5-0.5B-Instruct-GGUF \\
      ~/Downloads/qwen2.5-0.5b-instruct-q4_k_m.gguf
  ...
  # exact content:
  droste-hf-adopt.py ~/Downloads/qwen2.5-0.5b-instruct-q4_k_m.gguf

  # Adopt a whole diffusers checkout (nested dirs need --recursive),
  # reclaiming the disk space afterwards:
  droste-hf-adopt.py --apply --move --recursive \\
...
def die(msg, code=2):
    progress_clear()
    print(f"droste-hf-adopt.py: error: {msg}", file=sys.stderr)
    sys.exit(code)
